...
public abstract class ProcBase : MonoBehaviour
{
	...
	protected void BuildRingInverted(MeshBuilder meshBuilder, int segmentCount, Vector3 centre, float radius, float v, bool buildTriangles)
{
    float angleInc = (Mathf.PI * 2.0f) / segmentCount;
    ...
    for (int i = 0; i <= segmentCount; i++)
    {
        ...
        // Invert the normal vector to face inward
        Vector3 invertedNormal = -unitPosition;
        meshBuilder.Vertices.Add(centre + unitPosition * radius);
        meshBuilder.Normals.Add(invertedNormal); // Use the inverted normal
        meshBuilder.UVs.Add(new Vector2((float)i / segmentCount, v));
	...
	protected void BuildRingFlat(MeshBuilder meshBuilder, int segmentCount, Vector3 centre, float radius, float radius2, float v, bool buildTriangles)
{
    float angleInc = (Mathf.PI * 2.0f) / segmentCount;
    ...
    for (int i = 0; i <= segmentCount; i++)
    {
		...
		if(v==1){
         ...
         Vector3 vertexOuter = centre + unitPosition * radius;
        Vector3 vertexInner = centre + unitPosition * radius2;
        meshBuilder.Vertices.Add(vertexOuter);
        meshBuilder.Vertices.Add(vertexInner);
		meshBuilder.Normals.Add(unitPosition); // Use the inverted normal
        meshBuilder.UVs.Add(new Vector2((float)i / segmentCount, v));
  ...
  if (i > 0 && buildTriangles)
        {
            ...
            // Add triangles for the quad formed by adjacent vertices
		if(v==1){
				meshBuilder.AddTriangle(index1, index2, index0);
				meshBuilder.AddTriangle(index1, index3, index2);
		}else{
